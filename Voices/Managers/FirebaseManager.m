...
- (BOOL)shouldAddAction:(Action *)action toList:(NSMutableArray *)list {
    
    NSDate *currentTime = [NSDate date];
    
    if (action.timestamp < currentTime.timeIntervalSince1970) {
        
        NSInteger index = [list indexOfObjectPassingTest:^BOOL(Action *actionInArray, NSUInteger idx, BOOL *stop) {
